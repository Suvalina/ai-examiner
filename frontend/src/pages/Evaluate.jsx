...
const Evaluate = ({ setLoading }) => {
  ...
  const [newStudent, setNewStudent] = useState({ name: '', email: '', roll_number: '', class: '' });
  ...
  const handleAddStudent = async () => {
    ...
    try {
      const response = await createStudent(newStudent.name, newStudent.email, newStudent.roll_number, newStudent.class);
      setStudents([...students, response.student]);
      setSelectedStudent(response.student._id);
      setShowNewStudent(false);
      setNewStudent({ name: '', email: '', roll_number: '', class: '' });
    } catch (err) {
      setError(err.response?.data?.error || err.message);
    }
  };
